...
var ResponseCodes;
(function(ResponseCodes3) {
  ResponseCodes3[ResponseCodes3["NotFound"] = 404] = "NotFound";
  ResponseCodes3[ResponseCodes3["Error"] = 500] = "Error";
  ResponseCodes3[ResponseCodes3["Ok"] = 200] = "Ok";
  ResponseCodes3[ResponseCodes3["BadRequest"] = 400] = "BadRequest";
...
// functions/albums.ts
var API_KEY = "93ca745949bfbf9c7616c9359bc9cdae";
var handler = async (event, context) => {
  ...
  try {
    const url = `http://ws.audioscrobbler.com/2.0/?method=album.search&album=${encodeURI(name)}&api_key=${API_KEY}&format=json`;
    const response = await fetch(url);
    const jsonRes = await response.json();
    const matches = jsonRes.results.albummatches.album.map((al) => ({
      name: al.name,
      artist: al.artist,
      images: al.image.reduce((prev, cur) => [...prev, { url: cur["#text"], size: cur.size }], [])
    }));
    return sendResponse(ResponseCodes.Ok, { message: matches });
  } catch (e2) {
    return sendResponse(ResponseCodes.Error, { error: e2 });
  }
};
